...
namespace TestCsharp.Filters
{
    [AttributeUsage(AttributeTargets.Method, AllowMultiple = false)]
    public class AuthorizeUser : AuthorizeAttribute
    {
        private usuario oUsuario;
        private FundamentosCsharpEntities db = new FundamentosCsharpEntities();
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            try
            {
                ...
                var ltsMisOperaciones = from m in db.rol_operacion
                                        where m.idRol == oUsuario.idRol
                                        && m.idOperacion == idOperacion
                                        select m;

                if (ltsMisOperaciones.ToList().Count() == 0)
                {
